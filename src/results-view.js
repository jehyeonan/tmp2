...
export function renderResults(rows, activeFilter) {
  ...
  tbody.innerHTML = visible.map(row => `
    <tr class="${row['확인필요'] === '예' ? 'row-needs-check' : ''}">
      <td>${escapeHtml(row['팀명'])}</td><td>${escapeHtml(row['부서명'])}</td><td>${escapeHtml(row['제목'])}</td>
      <td class="numeric">${escapeHtml(row['처리기한'] || '-')}</td><td>${escapeHtml(row['우선순위'])}</td>
      <td><span class="tag ${row['확인필요'] === '예' ? 'needs-check' : 'complete'}">${escapeHtml(row['확인필요'])}</span></td>
      <td>${escapeHtml(row['정리메모'])}</td><td><button class="button secondary small-button" type="button" data-row-number="${Number(row._row)}">상세</button></td>
    </tr>
  `).join('');
}
